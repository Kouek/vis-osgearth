...
using namespace std;
using namespace SciVis::GraphViser2D;

Node::Node(int id, string data)
    : id{id}, data{data}, pos{Vec2D::random(1.0)}, vel{Vec2D::random(1.0)}, acc{Vec2D::random(1.0)},
      mass{1.0}, radius{1}, repulsion{1.0}, stiffness{1.0}, damping{1.0}, degree{0} {}

// Vec2D::Vec2D(double x, double y, double z) : x{x}, y{y}, z{z} {}
Vec2D::Vec2D(double x, double y) : x{x}, y{y} {}

double Vec2D::distance(Vec2D v1, Vec2D v2) {
    // double distance = sqrt(pow(v1.x - v2.x, 2.0) + pow(v1.y - v2.y, 2.0) + pow(v1.z -
    // v2.z, 2.0));
    double distance = sqrt(pow(v1.x - v2.x, 2.0) + pow(v1.y - v2.y, 2.0));
    return distance;
}

Vec2D Vec2D::zero() { return Vec2D(0.0, 0.0); }

Vec2D Vec2D::random(double range) {
...
Vec2D Universe::compute_spring_force(Node n1, Node n2) { return spring_k * (n2.pos - n1.pos); }
Vec2D Universe::compute_spring_force_general(double k, Vec2D v1, Vec2D v2) { return k * (v1 - v2); }

// Coloumb's Law: F_repulsion = k (q1 * q2) / r^2
Vec2D Universe::compute_repulsion_force(Node n1, Node n2) {
    double dist = Vec2D::distance(n1.pos, n2.pos);
    return repulsion * (1 / pow(dist, 2.0)) * Vec2D((n1.pos.x - n2.pos.x), (n1.pos.y - n2.pos.y));
}
...
void Universe::update(double deltaT) {
    ...
    for (int i = 0; i < this->graph.adj_list.size(); i++) {
        ...
        // Vec2D f_spring = Vec2D::random(10.0);
        // Vec2D f_repulsion = Vec2D::random(10.0);
        Vec2D f_spring = Vec2D::zero();
        Vec2D f_repulsion = Vec2D::zero();
        ...
        f_spring = f_spring - compute_spring_force_general(gravity, n1.pos, Vec2D::zero());

        // Integrate laws of motion
        Vec2D f_net = f_spring + f_repulsion;

        // Compute acceleration
        // a = F_net/m
        Vec2D new_acc = (f_net - n1.vel * damping) * (1.0 / n1.mass);
        Vec2D new_vel = n1.vel + new_acc * deltaT;
        Vec2D new_pos = n1.pos + new_vel * deltaT;
